...
# ---------- Pull config ----------
$FusedBin            = Resolve-ConfigPath (Get-TomlValue "fused_server" "binary")
...
$T2mBin              = Resolve-ConfigPath (Get-TomlValue "t2m_infer" "binary")
...
# Windows binaries live in build\Release\ by CMake convention. The config
# carries forward-slash paths ("build/t2m_infer") that work on POSIX; on
# Windows we have to retarget to the .exe under build\Release\ since the
# unified CMakeLists produces that layout for MSVC. If config.toml already
# points at an .exe we trust it as-is.
function Fix-WindowsBin($p, $stem) {
    if ($p -match '\.exe$' -and (Test-Path $p)) { return $p }
    $tryExe = "$p.exe"
    if (Test-Path $tryExe) { return $tryExe }
    # Common case: config says "build/fused_server" but msvc put it under build/Release/
    $alt = Join-Path (Split-Path -Parent $p) "Release\$stem.exe"
    if (Test-Path $alt) { return $alt }
    # Fallback: just append .exe and let preflight complain if it's wrong
    return $tryExe
}
$FusedBin = Fix-WindowsBin $FusedBin "fused_server"
$T2mBin   = Fix-WindowsBin $T2mBin   "t2m_infer"
...
$fail = $false
if (-not $SkipFused) {
    if (-not (Test-Path $FusedBin))   { Err "fused_server bin missing: $FusedBin"; $fail = $true }
    if (-not (Test-Path $FusedModel)) { Err "fused_server model missing: $FusedModel"; $fail = $true }
...
if (-not $SkipT2m) {
    if (-not (Test-Path $T2mBin))     { Err "t2m_infer bin missing: $T2mBin"; $fail = $true }
    if (-not (Test-Path $T2mOnnxDir)) { Err "t2m onnx dir missing: $T2mOnnxDir"; $fail = $true }
...
try {
    ...
    if ($TailLog) {
        Get-Content "$LogDir\t2m_infer.log" -Wait -Tail 0 &
    }
