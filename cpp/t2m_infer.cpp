...
//
// Backend: ONNX Runtime CPU EP only. No DirectML, no CoreML. Rationale:
//   - DiT is ~50M params, T<200, launch-bound on consumer GPUs
//   - CPU EP wins by 2x on the same hardware (measured on Windows)
//   - Apple Silicon CPU + Accelerate.framework BLAS performs very well here
//
...
struct Args {
    ...
    int         num_steps   = 4;
    int         seed        = 42;
    bool        use_cpu     = true;   // default true; --cpu is a no-op kept for CLI parity
};
...
static void usage(const char* a0) {
    ...
    std::printf("  --cpu              CPU EP (default; flag kept for parity)\n");
}
...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        std::string k = argv[i];
        if      (k == "--server")       a.server_mode = true;
        ...
        else if (k == "--cpu")          a.use_cpu   = true;  // already default
...
// ------------------------------------------------------------
// init: load weights + build ORT CPU sessions
// ------------------------------------------------------------
static void init_inference(InferContext& ctx, const std::string& onnx_dir) {
    ...
    ctx.sess_refiner = std::make_unique<Ort::Session>(
        ctx.env, ort_path(birefinerPath.string()).c_str(), so);
    ctx.sess_dit     = std::make_unique<Ort::Session>(
        ctx.env, ort_path(ditPath.string()).c_str(), so);

    if (ctx.verbose) std::printf("    loaded both onnx\n");
}
...
static int run_oneshot(const Args& args) {
    ...
    std::printf("\n[done] wrote %s  shape=(%u, %u)  abs_max=%.3f  abs_mean=%.4f\n",
...
static int run_server(const Args& args) {
    ...
    // Warmup (ORT CPU EP also benefits from a dummy run: triggers
    // mem-pattern planning and JIT'd kernels for the static-ish shapes)
    ...
    try {
        ...
        std::fprintf(stderr,
            "[server] warmup ok: refiner=%.2fms dit_total=%.2fms step0=%.2fms rest_avg=%.2fms\n",
