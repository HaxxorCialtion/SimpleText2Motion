...
static bool run_motion_to_vrma(const fs::path& bin,
...
#ifdef _WIN32
    // Windows: 用 CreateProcessW 直接启动，绕开 cmd.exe 的引号/反斜杠地狱。
    // 命令行按 Windows 规则用引号包裹每个含空格的参数即可。
    auto q = [](const std::wstring& s) -> std::wstring {
        return L"\"" + s + L"\"";
    };
    std::wstring wcmd =
        q(bin.wstring()) +
        L" --in "    + q(bin_in.wstring()) +
        L" --out "   + q(vrma_out.wstring()) +
        L" --title " + q(std::wstring(title_trunc.begin(), title_trunc.end()));
    ...
    if (code != 0) {
        err = "motion_to_vrma rc=" + std::to_string(code);
        return false;
    }
#else
    // POSIX: popen 原样保留。
